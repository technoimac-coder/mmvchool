        ...
        <aside className="space-y-4 overflow-auto border-l p-4">
          ...
          {canSign && pdf && <>
            ...
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-2 text-sm">
              ...
              <canvas ref={commentPad} width={600} height={200} aria-label="กระดานเขียนความคิดเห็น" className="w-full touch-none rounded-xl border-2 border-indigo-200 bg-white" style={{ aspectRatio: 3 }}
                onPointerDown={e => { const c = e.currentTarget; c.setPointerCapture(e.pointerId); commentDrawing.current = true; const r = c.getBoundingClientRect(); const ctx = c.getContext('2d')!; ctx.strokeStyle = '#263238'; ctx.lineWidth = penSize; ctx.lineCap = 'round'; ctx.lineJoin = 'round'; ctx.beginPath(); ctx.moveTo((e.clientX - r.left) * 600 / r.width, (e.clientY - r.top) * 200 / r.height); }}
              ...
              <p className="mt-1 text-[11px] text-slate-500">โหมดปากกาให้ลากเขียนลงบนหน้า PDF ได้ทันที เหมือนสมุดโน้ต</p>
              {commentPlacement && <div className="mt-1 text-[11px] text-slate-500">ลากลายมือบนเอกสารเพื่อย้ายตำแหน่งได้</div>}
              <label className="mt-2 block text-xs text-slate-600">ขนาดลายมือบนเอกสาร<input aria-label="ขนาดลายมือบนเอกสาร" type="range" min="0.08" max="1" step="0.01" value={commentPlacement?.width ?? 0.52} disabled={!commentPlacement} onChange={e => { const width = Number(e.target.value); setCommentPlacement(p => { if (!p) return p; const height = Math.min(1, p.height * width / p.width); const centerX = p.x + p.width / 2; const centerY = p.y + p.height / 2; return { ...p, width, height, x: Math.max(0, Math.min(1 - width, centerX - width / 2)), y: Math.max(0, Math.min(1 - height, centerY - height / 2)) }; }); setConfirmed(false); }} className="w-full" /></label>
            </div>
