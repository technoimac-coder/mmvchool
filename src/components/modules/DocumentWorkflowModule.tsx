...
export const DocumentWorkflowModule: React.FC = () => {
  ...
  return <div className="space-y-5">
    ...
    {selected && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/50 p-4"><div className="w-full max-w-2xl rounded-2xl bg-white p-5 shadow-2xl"><div className="flex items-center justify-between"><h2 className="font-bold">ตรวจสอบและลงนาม: {selected.title}</h2><button onClick={() => setSelected(null)} aria-label="ปิด"><X /></button></div><iframe className="mt-3 h-64 w-full rounded-xl border bg-slate-50" src={selected.fileUrl} title={selected.fileName} /><div className="mt-3 rounded-xl bg-slate-50 p-3 text-sm">ลำดับผู้ลงนาม: {selected.signers.map((s, index) => <span key={s.userId} className={index === selected.currentStep - 1 ? 'font-bold text-indigo-700' : ''}>{index ? ' → ' : ''}{s.userName} ({s.status === 'signed' ? 'ลงนามแล้ว' : s.status === 'rejected' ? 'ส่งกลับ' : 'รอดำเนินการ'})</span>)}</div>{selected.signers.some((s) => s.userId === currentUser.id && s.step === selected.currentStep && s.status === 'pending') && <><input className="mt-3 w-full rounded-xl border p-3" placeholder="พิมพ์ชื่อเพื่อยืนยันลายเซ็น" value={signature} onChange={(e) => setSignature(e.target.value)} /><div className="mt-3 flex gap-2"><button disabled={busy} onClick={() => void act('sign')} className="flex-1 rounded-xl bg-emerald-600 p-3 font-bold text-white"><CheckCircle2 className="mr-1 inline h-4 w-4" />ลงนามและส่งต่อ</button><button disabled={busy} onClick={() => void act('reject')} className="rounded-xl border border-rose-200 px-4 font-bold text-rose-600"><RotateCcw className="mr-1 inline h-4 w-4" />ส่งกลับ</button></div></>}</div></div>}
  </div>;
};
