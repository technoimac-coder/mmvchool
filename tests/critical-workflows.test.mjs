...
test('leave statistics are provided as a separate role-scoped module', () => {
  ...
  assert.match(module, /reportEndDate/);
  ...
  assert.match(module, /document\.fonts\.load\('400 16pt "TH SarabunPSK"', 'ชื่อ สกุล'\)/);
  assert.match(module, /document\.fonts\.load\('700 16pt "TH SarabunPSK"', 'ชื่อ สกุล'\)/);
  assert.match(module, /font-size: 16pt !important/);
  assert.match(module, /font-family: 'TH SarabunPSK'/);
  assert.match(module, /leave-statistics-name-column \{ width: 82mm !important/);
  assert.match(module, /leave-statistics-number-column \{ width: 11mm !important/);
  assert.match(module, /color: #000 !important/);
...
test('sidebar approval badges disappear after viewing and return only for new pending records', () => {
  ...
  assert.doesNotMatch(page, /pendingApprovalsCount > 0/);
